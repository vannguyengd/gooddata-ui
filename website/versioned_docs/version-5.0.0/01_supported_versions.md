...
2. **End-of-Development** (EOD)
    ...
    * **What it means:** A version in EOD receives only security fixes (unless they can be resolved by semver-compatible upgrade) and fixes for critical issues. Only production dependencies are applicable for security updates.  These fixes are applied on top of the last minor version. No new features are added.
