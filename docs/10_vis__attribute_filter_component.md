...
The **Attribute Filter component** is a dropdown component that lists attribute values. You can either pass a callback function, which receives a list of the selected values when a user clicks **Apply**, or the component handles the change after the click itself via ```connectToPlaceholder``` property.

If you use ```connectToPlaceholder``` property, the ```onApply``` is not needed and all is handled automatically. Use ```onApply``` only in case you need a specific callback to be fired.
