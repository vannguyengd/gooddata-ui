...
>  **The Dashboard component is still in the beta stage.**
>
>
...
| Name                     | Required? | Type                             | Description                                                                                                                                                                                               |
| :----------------------- | :-------- | :------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| menuButtonConfig         | false     | IMenuButtonConfiguration         | Optionally provide custom configuration for the Menu button.                                                                                                                                              |
| insightMenuItemsProvider | false     | InsightMenuItemsProvider         | Optionally provide custom provider to change items rendered in insight menus.                                                                                                                             |
...
## Selectors
You can obtain current `Dashboard` state values by using `useDashboardSelector` hook with relevant selector passed to it (all dashboard selectors are starting with `select` prefix). Selectors have the cache, so the reference equality of the return value should remain the same (unless the value itself changed) - this is useful to avoid unnecessary React re-renders.
...
## Commands
Commands offers a way how to control and change the current Dashboard state (e.g. the currently set filters). To dispatch a command, there is a hook called `useDashboardDispatch`. You should not alter the `Dashboard` state by directly mutating it as it can lead to undesired behavior and bugs. Always use commands. You can provide a custom correlation id to the command - all relevant events triggered by this command will contain it. 
